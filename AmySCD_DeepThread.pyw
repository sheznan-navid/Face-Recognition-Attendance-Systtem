...
encode_detector = cv2.dnn.readNetFromCaffe(protoPath, modelPath)
...
def intro(name_str):
    intr = 'Hey there, {}'.format(name_str)
    # q.put(intr)
    show.put(intr)
    speak()
...
def encode_faces():
    ...
    for (i, imagePath) in enumerate(image_paths):
        ...
        encode_detector.setInput(imageBlob)
        detections = encode_detector.forward()
        ...
        if len(detections) == 1:
            ...
            if confidence > 0.1:
                ...
                os.remove(imagePath)
                logging.info("No face found, Removed.")
        else:
            os.remove(imagePath)
            logging.info("No face or multiple found, Removed.")
...
def net_devices(net_dev):
    for device in net_dev:
        url = 'rtsp://admin:Ad2729$G@' + device + ':554/cam/realmonitor?channel=1&subtype=0&unicast=true&proto=Onvif'
        cam_thread = CamThread("Net Camera {}".format(net_dev.index(device) + 1), url)
        cam_thread.start()


def lunch_camera(camera_number):
...
def save_data(eid, name, first_entry, frame, person_no, date, ti):
    # db = mysql.connector.connect(host="localhost", user="root", passwd="", database="amy-ai")
    # sql = db.cursor()
    ...
    if first_entry and person_no == 1:
        cv2.imwrite(path + "\\dataset\\" + eid + "\\" + eid + " " + date + '.jpg',
                    frame)
    elif person_no > 1:
        cv2.imwrite(path + "\\employees\\" + eid + " " + date + ti + '.jpg', frame)
    else:
        store_dir = path + "\\detected\\" + date
        if not os.path.isdir(store_dir):
            os.mkdir(store_dir)
        cv2.imwrite(store_dir + "\\" + eid + " " + date + ti + '.jpg', frame)
...
def time_interval(eid, date):
    # db = mysql.connector.connect(host="localhost", user="root", passwd="", database="amy-ai")
    # sql = db.cursor()
    time_diff = 0.0
    try:
        sql.execute("SELECT MAX(TIME) FROM LOG WHERE ID='" + eid + "' and DATE='" + date + "'")
...
def recogFace(data, encoding):
    return face_recognition.compare_faces(data["encodings"], encoding, tolerance=0.4)


def recogFacedist(data, encoding):
    return face_recognition.face_distance(data["encodings"], encoding)
...
def face_identifier(encodings):
    ...
    for encoding in encodings:
        ...
        matches = pool3.apply_async(recogFace, (data, encoding,)).get()
        # print(matches)
        identity = "Unknown"
        if 1 < matches.count(True) < 4:
            # Or instead, use the known face with the smallest distance to the new face
            best_match_indexes = []
            face_distances = pool3.apply_async(recogFacedist, (data, encoding,)).get()
            best_faces = face_distances[np.argpartition(face_distances, 5)[:5]]
            print("Best Face Distances: {}".format(best_faces))
            for i in best_faces:
                best_match_indexes.append(list(face_distances).index(i))
            print("Best Face Indexes: {}".format(best_match_indexes))
            for i in best_match_indexes:
                if face_distances[i] < 0.52:
                    matches[i] = True
        ...
        if matches.count(True) >= 4:
            ...
            for i in matched_ids:
                matched_id = data["names"][i]
                counts[matched_id] = counts.get(matched_id, 0) + 1

            # determine the recognized face with the largest number
            # of votes (note: in the event of an unlikely tie Python
            # will select first entry in the dictionary)
            matched_id = max(counts, key=counts.get)
            if counts[matched_id] >= 4:
                identity = matched_id
...
def recog_encodings(rgb, boxes):
    return face_recognition.face_encodings(rgb, boxes)


def entry_monitor(ids, date):
    first_entries = {}
    # db = mysql.connector.connect(host="localhost", user="root", passwd="", database="amy-ai")
    # sql = db.cursor()
    for i in ids:
...
def face_stream(camera, src):
    ...
    while True:
        ...
        try:
            ...
            cv2.destroyAllWindows()
            vs.stop()
            lunch_camera(devices())
            net_devices(net_dev)
            continue
        # update the FPS counter
        fps.update()
        if Scene.detectChange(gray, camera):
            # detect faces in the gray scale frame
            rects = pool1.apply_async(dect_face, (frame,)).get()
            # OpenCV returns bounding box coordinates in (x, y, w, h) order
            # but we need them in (top, right, bottom, left) order, so we
            # need to do a bit of reordering
            f_date = datetime.datetime.now().strftime("%Y-%m-%d")
            f_time = datetime.datetime.now().strftime(" %H.%M.%S")
            faces = len(rects)
            if faces:
                pronounce = ''
                names = ''
                ...
                rgb = frame[:, :, ::-1]
                encodings = ThreadPool(processes=faces).apply_async(recog_encodings, (rgb, rects,)).get()
                ids = ThreadPool(processes=faces).apply_async(face_identifier,(encodings,)).get()
                ...
                if len(ids) == 1 and ids[0] == "Unknown" and interval > 1:
                    # greet_unknown(frame)
                    store_dir = path + "\\Unknown\\" + f_date
                    if not os.path.isdir(store_dir):
                        os.mkdir(store_dir)
                    cv2.imwrite(store_dir + "\\" + "Unknown" +
                                " " + f_date + f_time + '.jpg', frame)
                    last_cap = time.time()
                else:

                    first_entries = pool5.apply_async(entry_monitor, (ids, f_date,)).get()
                    ...
                    if pronounce:
                        ...
                        if True in first_entries.values():
                            greet = Thread(target=greeting, args=(name_str, ac_name_str,))
                            greet.start()
                            '''
                        for i in ids:
                            time_inter = pool3.apply_async(time_interval, (i, f_date,)).get()
                            names_no = len(rects)
                            ...
                            if i != "Unknown" and (time_inter > 2.0 or first_entries[i] is True):
                                save_data(i, names[i], first_entries[i], frame, names_no, f_date, f_time)
                                '''
                                if not first_entries[i]:
                                    mention = Thread(target=intro, args=(ac_name_str,))
                                    mention.start()
                                    '''
                            elif i == "Unknown" and interval > 1:
                                # greet = Thread(target=greeting, args=("Unknown", "Unknown", frame))
                                # greet.start()
                                # greet.join()
                                last_cap = time.time()
                            else:
                    ...
                    for ((startY, endX, endY, startX), name) in zip(rects, names.values()):

                        # draw the predicted face name on the image
                        cv2.rectangle(frame, (startX * f_mul, startY * f_mul), (endX * f_mul, endY * f_mul),
                                      (0, 255, 0), 2)
                        y = startY - 10 if startY - 10 > 10 else startY + 10
                        cv2.putText(frame, name, (startX * f_mul, y), cv2.FONT_HERSHEY_SIMPLEX,
                                    0.75, (0, 255, 0), 2)

            else:
                store_dir = path + "\\Motion\\" + f_date
                if not os.path.isdir(store_dir):
                    os.mkdir(store_dir)
                cv2.imwrite(store_dir + "\\" + f_date + f_time + '.jpg', frame)
        # update the FPS counter
        fps.update()
...
if __name__ == "__main__":
    try:
        folders = ["Motion", "dataset", "trained_dataset", "detected", "employees", "Unknown", "logs"]
